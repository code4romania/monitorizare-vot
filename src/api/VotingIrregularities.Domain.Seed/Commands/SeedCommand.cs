...
namespace VotingIrregularities.Domain.Seed.Commands
{
    public class SeedCommand : Command
    {
        ...
        private readonly Dictionary<string, bool> FormsArray = new Dictionary<string, bool>()
        {
            { "A",false},
            { "B",true},
            { "C",true},
        ...
        public void SeedData()
        {
            using (var tran = _context.Database.BeginTransaction())
            {
                ...
                {
                    SeedForms(form.Key, form.Value);
                    SeedFormSections(form.Key);
                    SeedQuestions(form.Key, form.Value);
                }
                SeedObservers();
        ...
        {
            _context.Database.ExecuteSqlRaw("delete from NotesAttachments");
            _context.Database.ExecuteSqlRaw("delete from OptionsToQuestions");
            _context.Database.ExecuteSqlRaw("delete from Questions");
            _context.Database.ExecuteSqlRaw("delete from FormSections");
            _context.Database.ExecuteSqlRaw("delete from Forms");
            _context.Database.ExecuteSqlRaw("delete from Counties");
            _context.Database.ExecuteSqlRaw("delete from Observers");
        }
        ...
        private void SeedFormSections(string formCode)
        {
            var form = _context.Forms.SingleOrDefault(f => f.Code == formCode);
            if (form == null)
            {
                return;
            }

            _context.FormSections.AddRange(
        ...
        private void SeedQuestions(string formCode, bool diaspora)
        {
            var f = _context.Forms.FirstOrDefault(ff => ff.Code == formCode);
            if (f == null)
            {
                f = new Form { Code = formCode, Diaspora = diaspora, Draft = false };
                _context.Forms.Add(f);
                _context.SaveChanges();
            }

            var fsB = _context.FormSections
                .FirstOrDefault(ff => ff.IdForm == f.Id && ff.Code == $"{formCode}B");
            var fsC = _context.FormSections
        ...
        private void SeedForms(string formCode, bool diaspora)
        {
            _context.Forms.Add(
