...
{
    public class GetNgoAdminDetailsTests : EndpointsTests
    {
        public GetNgoAdminDetailsTests(CustomWebApplicationFactory factory) : base(factory)
        {
        }
        ...
        public async Task Returns_Success_For_Valid_Request()
        {
            ...
            // Act
            var response = await client.GetAsync($"/api/v1/ngo/1/ngoadmin/{id}");
        ...
        public async Task Prevents_Anonymous_Access()
        {
            ...
            // Act
            var response = await client.GetAsync("/api/v1/ngo/1/ngoadmin/1");
        ...
        public async Task Forbids_Unauthorized_Access_For_Other_Ngo_Admins()
        {
            ...
            // Act
            var response = await client.GetAsync($"/api/v1/ngo/{otherNgoAdminId}/ngoadmin/1");

            // Assert
            response.Should().Be400BadRequest()
                .And.BeAs(new
                {
                    detail = "Cannot get admins that are not in your Ngo"
                });
        }
    }
}